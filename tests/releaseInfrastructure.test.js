...
describe('release infrastructure hardening', () => {
  test('enforces real 100 percent runtime coverage for lines, branches and functions', () => {
    ...
    for (const gate of ['--test-coverage-lines=100', '--test-coverage-branches=100', '--test-coverage-functions=100', 'Number(value) !== 100']) assert.match(runner, new RegExp(gate.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
    const vitest = read('vitest.config.mjs');
    const scopeAudit = read('scripts/audit-coverage-scope.mjs');
    for (const metric of ['lines: 100', 'branches: 100', 'functions: 100', 'statements: 100']) assert.match(vitest, new RegExp(metric));
  ...
  test('uses Supabase CLI setup and executable pgTAP database assertions in CI', () => {
    ...
    for (const file of ['supabase/tests/order_security.sql', 'supabase/tests/enterprise_workflows.sql']) {
      const sql = read(file);
  ...
  test('ships generic factory specifications and fail-closed production preflight artifacts', () => {
    ...
    for (const token of ['FACTORY_TEMPLATE_SPECS', 'minimumRasterDpi', 'manual_factory_match_required', 'readyForManufacturing']) assert.match(preflight, new RegExp(token));
    for (const token of ['preflight.json', 'factory-specification.json', 'color-specifications.csv']) assert.match(exports, new RegExp(token.replace('.', '\\.')));
