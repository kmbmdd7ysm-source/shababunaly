...
import { buildDesignViewSvg, buildProductionPackage, productShape } from '../src/utils/designExports.js';
...
describe('production artwork exports', () => {
  ...
  it('uses a safe jersey fallback for an unknown production preview', () => {
    ...
    expect(productShape('unknown', '#000000', '#ffffff', '#cccccc', 'side')).toContain('scale(.58 1)');
  });

  it('packages SVG views, manifest and roster into a valid ZIP', async () => {
    const design = { ...DEFAULT_CUSTOM_DESIGN, productType: 'game-set' };
    const blob = buildProductionPackage({ design, studio: createDefaultStudio(design), roster: [{ name:'One',number:'7',jerseySize:'L',shortsSize:'L' }], reference: 'D-100' });
  ...
  it('creates multi-page proof and tech-pack PDFs', async () => {
    const design = { ...DEFAULT_CUSTOM_DESIGN, productType: 'game-set' };
    const docs = downloadDesignDocuments({ design, studio: createDefaultStudio(design), productLabel: 'Full Game Set', reference: 'D-100' });
