...
describe('Formspree server adapter', () => {
  it('normalizes every Formspree endpoint and payload helper branch', () => {
    ...
    expect(sanitize({ a: 1 })).toContain('"a": 1');
    expect(sanitizeKey(undefined)).toBe('');
    expect(sanitizeKey(' bad key!? ')).toBe('bad_key__');
    expect(sanitizeKey('a'.repeat(100))).toHaveLength(80);
    expect(buildCleanFormPayload(null)).toEqual({});
    expect(buildCleanFormPayload({ turnstileToken: 'secret', '': 'skip', ok: 'yes' })).toEqual({
      ok: 'yes',
    });
  });
  ...
  it.each([400, 429, 500])('does not report success when provider returns %s', async (status) => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({ ok: false, status, text: async () => 'rejected' }),
    );
    const res = responseMock();
    await handler(
      request({
  ...
  it('reports delivery failure on timeout/network rejection', async () => {
    ...
    expect(res.statusCode).toBe(502);
    expect(res.body.error).toBe('formspree_delivery_failed');
  });
});
