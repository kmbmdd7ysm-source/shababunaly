...
import {
  addDesignComment, addDesignLayer, createDefaultStudio, createHistory, duplicateDesignLayer, moveDesignLayer,
  normalizeStudio, pushHistory, redoHistory, removeDesignLayer, undoHistory, updateDesignLayer,
} from '../src/services/designStudio.js';

const design = { teamName: 'AL ITTIHAD', playerName: 'PLAYER', number: '21', secondary: '#ffffff', accent: '#d6d6d6', font: 'block' };
...
describe('production design studio state', () => {
  it('creates editable front and back production layers', () => {
    ...
    expect(studio.layers.some((layer) => layer.view === 'front' && layer.type === 'number')).toBe(true);
    expect(studio.layers.some((layer) => layer.view === 'back' && layer.type === 'number')).toBe(true);
  });
  it('clamps unsafe layer coordinates and supports layer operations', () => {
    let studio = addDesignLayer(createDefaultStudio(design), { type: 'text', content: 'TEST', x: 300, y: -20 }, design);
    const added = studio.layers.at(-1);
    expect(added.x).toBe(97); expect(added.y).toBe(3);
  ...
  it('maintains bounded undo and redo history', () => {
    ...
    history = undoHistory(history); expect(history.present.layers[0].x).toBe(initial.layers[0].x);
    history = redoHistory(history); expect(history.present.layers[0].x).toBe(72);
  });
  it('pins review comments to a named view', () => {
    const studio = addDesignComment(createDefaultStudio(design), { view: 'side', x: 30, y: 40, text: 'Move sponsor higher.' }, design);
