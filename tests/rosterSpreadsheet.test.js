...
async function minimalRosterXlsx() {
  const files = [
    { name: '[Content_Types].xml', data: '<?xml version="1.0"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"></Types>' },
    { name: 'xl/workbook.xml', data: '<?xml version="1.0"?><workbook xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Roster" sheetId="1" r:id="rId1"/></sheets></workbook>' },
    { name: 'xl/_rels/workbook.xml.rels', data: '<?xml version="1.0"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Target="worksheets/sheet1.xml"/></Relationships>' },
...
describe('roster spreadsheet import', () => {
  it('imports XLSX rows without a third-party spreadsheet runtime', async () => {
    ...
    expect(rows[0]).toMatchObject({ name: 'Seddig Etorki', jerseyName: 'ETORKI', number: '20', jerseySize: 'L', shortsSize: 'L' });
  });

  it('imports CSV and rejects unsupported files', async () => {
    const csv = new File(['Player Name,Number,Jersey Size,Shorts Size\nOne,7,L,L'], 'roster.csv', { type: 'text/csv' });
    ...
    let error = null;
    try { await parseRosterFile(unsupported); } catch (caught) { error = caught; }
