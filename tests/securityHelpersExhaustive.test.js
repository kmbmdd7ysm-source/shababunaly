...
function req(body = {}, headers = {}, socket = {}) {
  return {
    body,
    headers: {
    ...
    socket: { remoteAddress: '127.0.0.2', ...socket },
  };
}
...
describe('request security helpers exhaustive', { concurrency: false }, () => {
  ...
  it('rejects bad origins, oversized requests and quietly consumes honeypots', async () => {
    ...
    const origin = res();
    expect(await guardPublicRequest(req({}, { origin: 'https://evil.example' }), origin)).toBe(
      false,
    );
    expect(origin.statusCode).toBe(403);
    const size = res();
    expect(
      await guardPublicRequest(req({}, { 'content-length': '99' }), size, { maxBytes: 10 }),
    ).toBe(false);
    expect(size.statusCode).toBe(413);
    const honeypot = res();
    expect(await guardPublicPost(req({ website: 'bot' }), honeypot)).toBe(false);
    expect(honeypot.statusCode).toBe(200);
    const malformed = res();
    expect(
      await guardPublicPost(req('text'), malformed, { honeypot: false, bucket: 'malformed' }),
    ).toBe(true);
  });
  ...
  it('uses durable limits for allow and deny decisions', async () => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({ ok: true, status: 200, json: async () => true }),
    );
    const allowed = res();
    expect(await guardPublicRequest(req(), allowed, { bucket: 'durable-allow' })).toBe(true);
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({ ok: true, status: 200, json: async () => false }),
    );
    const denied = res();
    expect(
      await guardPublicRequest(req(), denied, { bucket: 'durable-deny', windowMs: 1001 }),
    ).toBe(false);
  ...
  it('fails closed in production when durable protection is unavailable', async () => {
    process.env.NODE_ENV = 'production';
    const absent = res();
    expect(await guardPublicRequest(req(), absent, { bucket: 'prod-absent' })).toBe(false);
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({ ok: false, status: 500, json: async () => false }),
    );
    const failed = res();
    expect(await guardPublicRequest(req(), failed, { bucket: 'prod-failed' })).toBe(false);
  ...
  it('uses and exhausts the development fallback bucket', async () => {
    ...
    const second = res();
    expect(
      await guardPublicRequest(req(), first, { bucket: 'dev-once', limit: 1, windowMs: 60000 }),
    ).toBe(true);
    expect(
      await guardPublicRequest(req(), second, { bucket: 'dev-once', limit: 1, windowMs: 60000 }),
    ).toBe(false);
...
describe('Supabase REST helpers exhaustive', { concurrency: false }, () => {
  ...
  it('handles JSON, text, empty, binary and upstream errors for admin requests', async () => {
    ...
    expect(calls[3][1].headers['Content-Type']).toBe(undefined);
    expect(calls[3][1].headers['X-Test']).toBe('yes');
    let error;
    try {
      await supabaseAdminRequest('/error');
    } catch (caught) {
      error = caught;
    }
  ...
  it('resolves valid users and safely handles missing, denied, invalid and failed auth', async () => {
    ...
    let failed = false;
    try {
      await resolveSupabaseUser('Bearer c');
    } catch (e) {
      failed = String(e.message).includes('supabase_auth:500');
    }
  ...
  it('performs user-scoped requests with all response forms', async () => {
    ...
    expect(
      await supabaseUserRequest('/json', 'Bearer user', { method: 'POST', body: '{}' }),
    ).toEqual({ id: 1 });
    expect(await supabaseUserRequest('/text', 'user')).toBe('text');
    expect(await supabaseUserRequest('/empty', 'user')).toBe(null);
    expect(
      await supabaseUserRequest('/binary', 'user', {
        body: new Uint8Array([1]),
        headers: { 'X-Test': 'ok' },
      }),
    ).toEqual({});
    expect(calls[0][1].headers.Authorization).toBe('Bearer user');
    expect(calls[3][1].headers['Content-Type']).toBe(undefined);
    let error;
    try {
      await supabaseUserRequest('/error', 'user');
    } catch (caught) {
      error = caught;
    }
...
describe('Turnstile and payment registry exhaustive', { concurrency: false }, () => {
  ...
  it('normalizes adapter IDs and reports only configured methods', () => {
    ...
    expect(getPaymentAdapter('missing')).toBe(null);
    expect(getPaymentAdapter(null)).toBe(null);
    expect(configuredPaymentMethods()).toEqual([]);
    process.env.PAYMENTS_SESSION_URL = 'https://pay.example';
    process.env.PAYMENTS_SECRET_KEY = 'secret';
    process.env.LIBYAN_BANK_CARD_SESSION_URL = 'https://bank.example';
    process.env.LIBYAN_BANK_CARD_SECRET_KEY = 'secret';
