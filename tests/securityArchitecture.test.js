...
describe('trusted checkout architecture', () => {
  it('saves the trusted order before creating a hosted payment session', () => {
    ...
    expect(source.indexOf('const confirmation = await savePendingOrder(payload)')).toBeGreaterThan(-1);
    expect(source.indexOf('createCheckoutSession({')).toBeGreaterThan(source.indexOf('const confirmation = await savePendingOrder(payload)'));
  });
  ...
  it('enforces shipping, wholesale and staged payment in SQL', () => {
    const sql = fs.readFileSync('supabase/migrations/20260731020000_shababuna_global_commerce.sql', 'utf8');
