...
import { CUSTOM_PRODUCT_TYPES, DEFAULT_CUSTOM_DESIGN } from '../src/data/customization.js';
...
function completeStudio(design = completeDesign) {
  ...
  return {
    ...studio,
    layers: studio.layers.map((layer) => ({ ...layer, x: 50, y: 50, width: 10, content: layer.content || 'TEXT' })),
    comments: [],
  };
}
...
describe('production preflight and manufacturing specifications', () => {
  ...
  test('converts validated print colors to RGB, CMYK and CSV without inventing a spot color', () => {
    ...
    const png = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAIAAAADCAIAAADZSiLoAAAAA3NCSVQICAjb4U/gAAAADElEQVR4nGP4z8AAAAMBAQDJ/pLvAAAAAElFTkSuQmCC';
    assert.deepEqual(readRasterDimensions(png), { pixelWidth: 2, pixelHeight: 3, format: 'png', source: 'embedded_header' });
    ...
    assert.equal(invalid.every((item) => item.hex === null && item.rgb === null && item.cmyk === null), true);
  ...
  test('passes quote preflight while requiring real factory proof before manufacturing', () => {
    const preflight = runProductionPreflight({ design: completeDesign, studio: completeStudio(), roster: [] });
    ...
    const colors = buildColorSpecifications(completeDesign);
    const approved = runProductionPreflight({ design: completeDesign, studio: completeStudio(), factoryApproval: {
      approved: true, approvalStatus: 'approved', profileId: 'factory-approved-v1', manufacturer: 'Approved Factory', manufacturerLegalId: 'LY-FACTORY-001',
      certificateReference: 'CERT-001', certificateSha256: 'a'.repeat(64), templateVersion: 'generic-production-v2', productTypes: ['game-set'],
      iccProfileReference: 'ICC-001', iccProfileSha256: 'b'.repeat(64), pantoneLibrary: 'Pantone Solid Coated', pantoneLibraryVersion: '2026', pantoneLibrarySha256: 'c'.repeat(64),
      deltaETolerance: 2, gradedPatternSha256: 'd'.repeat(64), materialProfile: { fabricCode: 'SUB-220', stretchPercent: 4, shrinkagePercent: 1 },
      colorMeasurements: colors.map((color) => ({ role: color.role, targetLab: rgbToLab(color.rgb), measuredLab: rgbToLab(color.rgb) })),
    } });
    assert.equal(approved.readyForManufacturing, true);
    assert.equal(approved.status, 'factory_approved');
    assert.equal(approved.warnings.some((item) => item.code === 'factory_approval_required'), false);
  });

  test('blocks invalid quantity, colors, artwork, roster and empty text layers', () => {
    const design = { ...completeDesign, quantity: 1, primary: 'bad', secondary: '', accent: '#12345g', notes: '' };
    const studio = {
      ...completeStudio(design),
      layers: [
        { id: 'blank', type: 'text', view: 'front', label: 'Blank', content: '', x: 50, y: 50, width: 10, rotation: 0, color: '#fff000', font: 'block', visible: true, locked: false, zIndex: 1 },
      ],
      comments: [{ id: 'c1', view: 'front', x: 2, y: 2, text: 'Resolve me', resolved: false, createdAt: '2026-01-01' }],
    };
    ...
    const noArtwork = runProductionPreflight({ design: completeDesign, studio: { ...completeStudio(), layers: completeStudio().layers.map((layer) => ({ ...layer, visible: false })) } });
  ...
  test('flags unsafe placement and raster assets with measurable print requirements', () => {
    const studio = {
      ...completeStudio(),
      layers: [
        { id: 'raster', type: 'logo', view: 'front', label: 'Logo', content: 'data:image/png;base64,QUJDRA==', x: 2, y: 99, width: 90, rotation: 0, color: '#ffffff', font: 'block', visible: true, locked: false, zIndex: 1 },
        { id: 'empty-raster', type: 'logo', view: 'front', label: 'Empty raster payload', content: 'data:image/png;base64,', x: 50, y: 50, width: 20, rotation: 0, color: '#ffffff', font: 'block', visible: true, locked: false, zIndex: 2 },
        { id: 'hidden', type: 'logo', view: 'front', label: 'Hidden', content: 'data:image/png;base64,QUJDRA==', x: 50, y: 50, width: 20, rotation: 0, color: '#ffffff', font: 'block', visible: false, locked: false, zIndex: 3 },
        { id: 'external', type: 'logo', view: 'back', label: 'External', content: 'https://example.com/logo.png', x: 50, y: 50, width: 20, rotation: 0, color: '#ffffff', font: 'block', visible: true, locked: false, zIndex: 4 },
      ],
    };
    const preflight = runProductionPreflight({ design: completeDesign, studio });
    assert.equal(preflight.rasterAssets.length, 2);
    assert.equal(preflight.rasterAssets.some((item) => item.approximateBytes === 0), true);
  ...
  test('handles accessory products without applying roster blockers and uses defaults safely', () => {
    ...
    const preflight = runProductionPreflight({ design, studio: completeStudio(design), roster: [{ name: '', number: '', jerseySize: '' }] });
    assert.equal(preflight.blockers.some((item) => item.code === 'roster_validation'), false);
