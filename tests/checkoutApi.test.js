...
import handler from '../api/create-session.ts';
...
function installFetch(
  trustedOrder = order(),
  session = { url: 'https://payments.example/checkout/1', id: 'session-1' },
  orderStatus = 200,
  sessionStatus = 200,
) {
  const calls = [];
    ...
    vi.fn().mockImplementation(async (url, options = {}) => {
      ...
      if (target.includes('/rest/v1/orders?'))
        return {
          ...
          status: orderStatus,
          json: async () => (trustedOrder ? [trustedOrder] : []),
        };
...
describe('trusted checkout session API', { concurrency: false }, () => {
  it('rejects methods, oversized bodies and malformed references before provider access', async () => {
    ...
    await handler(request({}, 'POST', { 'content-length': '16001' }), size);
    expect(size.statusCode).toBe(413);
    const invalid = responseMock();
    await handler(request({ paymentMethod: 'cash', orderNumber: 'bad' }), invalid);
    expect(invalid.statusCode).toBe(400);
  });

  it('fails closed when payment or trusted order storage is unavailable', async () => {
    const provider = responseMock();
    await handler(
      request({ paymentMethod: 'online_card', orderNumber: 'SHB-20260801-0000001' }),
      provider,
    );
    expect(provider.statusCode).toBe(503);
    expect(provider.body.error).toBe('payment_provider_not_connected');
    configure();
    delete process.env.SUPABASE_URL;
    delete process.env.SUPABASE_SERVICE_ROLE_KEY;
    const store = responseMock();
    await handler(
      request({ paymentMethod: 'online_card', orderNumber: 'SHB-20260801-0000001' }),
      store,
    );
    expect(store.statusCode).toBe(503);
    expect(store.body.error).toBe('trusted_order_store_not_connected');
  });
  ...
  it('returns trusted lookup errors and never trusts browser totals', async () => {
    ...
    await handler(
      request({ paymentMethod: 'online_card', orderNumber: 'SHB-20260801-0000001', total: 1 }),
      missing,
    );
    expect(missing.statusCode).toBe(404);
    expect(missing.body.error).toBe('trusted_order_not_found');
    configure();
    installFetch(order(), {}, 502);
    const lookup = responseMock();
    await handler(
      request({ paymentMethod: 'online_card', orderNumber: 'SHB-20260801-0000001' }),
      lookup,
    );
    expect(lookup.statusCode).toBe(502);
    expect(lookup.body.error).toBe('trusted_order_lookup_failed');
  });
  ...
  it('blocks every non-payable order state and customer mismatch', async () => {
    ...
    expect(mismatch.body.error).toBe('order_customer_mismatch');
  });
  ...
  it('creates a provider session from the server-trusted amount and immutable identity', async () => {
    ...
    expect(body.trustedOrder).toMatchObject({
      orderNumber: 'SHB-20260801-0000001',
      currency: 'USD',
      amount: 110,
      amountMinor: 11000,
    });
    expect(providerCall[1].headers['Idempotency-Key']).toContain(
      '11111111-1111-4111-8111-111111111111:initial:0.00:110.00',
    );
  });

  it('supports the configured Libyan card adapter and maps provider failures safely', async () => {
    configure('libyan_bank_card');
    installFetch(order({ payment_method: 'libyan_bank_card' }), {
    ...
    configure();
    installFetch(order(), { code: 'declined' }, 200, 422);
    ...
    expect(failed.body.error).toBe('provider_rejected');
  });
});
