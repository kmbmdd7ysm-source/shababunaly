...
export const resolveOrderNotificationEndpoint = () => String(process.env.FORMSPREE_ORDER_ENDPOINT || process.env.VITE_FORM_ENDPOINT || '').trim();
...
async function loadTrustedOrder(orderNumber) {
  ...
  const select = ['order_number','customer_email','currency','subtotal','shipping_total','tax_total','discount_total','total','payment_method','payment_plan','amount_due_now','remaining_balance','order_status','payment_status','items_snapshot','shipping_summary','created_at'].join(',');
  const response = await fetch(`${base}/rest/v1/orders?select=${select}&order_number=eq.${encodeURIComponent(orderNumber)}&limit=1`, {
...
export default async function handler(req, res) {
  ...
  if (!endpoint || !/^https:\/\//i.test(endpoint)) return res.status(503).json({ ok: false, error: 'formspree_not_configured' });
  const input = req.body && typeof req.body === 'object' ? req.body : {};
  if (!input.orderNumber || !input.message) {
    return res.status(400).json({ ok: false, error: 'missing_order_payload' });
  }
  let lookup;
  try { lookup = await loadTrustedOrder(safe(input.orderNumber, 80).toUpperCase()); }
  catch { return res.status(502).json({ ok: false, error: 'trusted_order_lookup_failed' }); }
  if (lookup.configured && !lookup.order) return res.status(404).json({ ok: false, error: 'trusted_order_not_found' });
  ...
  const authoritativeMessage = trusted ? JSON.stringify({
