...
async function supabaseAuth(path, options = {}) {
  ...
  if (!base || !serviceKey) throw Object.assign(new Error('supabase_not_configured'), { status: 503 });
  ...
  if (!response.ok) throw Object.assign(new Error(`supabase_auth:${response.status}:${text.slice(0, 300)}`), { status: response.status >= 500 ? 502 : response.status });
  return data;
}

async function requireSuperAdmin(req) {
  const session = await requireStaffSession(req, { roles: new Set(['super_admin']), requireAal2: true });
...
export default async function handler(req, res) {
  ...
  if (Number(req.headers['content-length'] || 0) > 16_000) return json(res, 413, { ok: false, error: 'request_too_large' });
  let auth;
  try { auth = await requireSuperAdmin(req); }
  catch (error) { return json(res, error.status || 401, { ok: false, error: clean(error.message, 300) }); }
  ...
  try {
    if (req.method === 'GET') {
      ...
      const data = await supabaseAuth(`admin/users?page=${page}&per_page=${perPage}`, { serviceKey: auth.serviceKey });
      const users = Array.isArray(data?.users) ? data.users.map(sanitizeUser) : [];
      return json(res, 200, { ok: true, users, page, perPage, total: Number(data?.aud || data?.total || users.length) });
    }

    const body = req.body && typeof req.body === 'object' ? req.body : {};
    const userId = clean(body.userId, 100);
    const role = clean(body.role, 40).toLowerCase();
    if (!/^[0-9a-f-]{36}$/i.test(userId) || !ALLOWED_ROLES.has(role)) return json(res, 400, { ok: false, error: 'invalid_role_update' });
    if (userId === auth.requester.id && role !== 'super_admin') return json(res, 409, { ok: false, error: 'cannot_remove_own_super_admin_role' });

    const current = await supabaseAuth(`admin/users/${encodeURIComponent(userId)}`, { serviceKey: auth.serviceKey });
    const currentMetadata = current?.app_metadata && typeof current.app_metadata === 'object' ? current.app_metadata : {};
    ...
    const updated = await supabaseAuth(`admin/users/${encodeURIComponent(userId)}`, {
      method: 'PUT', serviceKey: auth.serviceKey, body: { app_metadata: nextMetadata },
    });
