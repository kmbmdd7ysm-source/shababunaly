...
const formspreeEndpoint = () => clean(process.env.FORMSPREE_ORDER_ENDPOINT || process.env.VITE_FORM_ENDPOINT, 1000);
...
async function deliver(row) {
  ...
  if (!/^https:\/\/formspree\.io\/f\/[A-Za-z0-9_-]+$/u.test(endpoint)) throw new Error('formspree_not_configured');
  ...
  const params = new URLSearchParams({
    ...
    reference_id: clean(payload.orderNumber || payload.quoteNumber || payload.requestNumber || row.entity_id, 160),
    ...
    details: clean(payload.description || payload.details || payload.note || payload.staffNotes, 5000),
  ...
  for (const [key, value] of Object.entries(payload).slice(0, 50)) {
    ...
    if (field && !params.has(field)) params.set(field, clean(typeof value === 'object' ? JSON.stringify(value) : value, 5000));
  }
  const response = await fetch(endpoint, { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }, body: params.toString() });
  const text = await response.text();
...
export default async function handler(req, res) {
  ...
  try {
    let expiredOrders = 0;
    try {
      const expiredResult = await supabaseRequest('rpc/expire_stale_commerce_orders', { method: 'POST', body: '{}' });
    ...
    const rows = await supabaseRequest('rpc/claim_commerce_notifications', { method: 'POST', body: JSON.stringify({ p_limit: 25 }) });
    let sent = 0;
    let failed = 0;
    for (const row of rows || []) {
      try {
        await deliver(row);
        await supabaseRequest(`commerce_notifications?id=eq.${encodeURIComponent(row.id)}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ delivery_status: 'sent', sent_at: new Date().toISOString(), last_error: null, updated_at: new Date().toISOString() }) });
        sent += 1;
      } catch (error) {
        ...
        await supabaseRequest(`commerce_notifications?id=eq.${encodeURIComponent(row.id)}`, {
          ...
          body: JSON.stringify({
            ...
            available_at: exhausted ? new Date('9999-12-31T23:59:59.000Z').toISOString() : new Date(Date.now() + delayMinutes * 60_000).toISOString(),
        ...
        if (exhausted) {
          try {
            await supabaseRequest('security_events', {
              ...
              headers: { Prefer: 'return=minimal' },
              body: JSON.stringify({
                event_type: 'notification_dead_letter', severity: 'high', source: 'notification-worker',
                entity_type: row.entity_type, entity_id: String(row.entity_id || row.id),
                details: { notificationId: row.id, eventType: row.event_type, attempts, error: clean(error.message, 1000) },
              }),
            });
          } catch { /* The dead-letter record remains visible even if alert creation fails. */ }
        }
        failed += 1;
      }
    }
    return json(res, 200, { ok: true, expiredOrders, processed: (rows || []).length, sent, failed });
  } catch (error) {
