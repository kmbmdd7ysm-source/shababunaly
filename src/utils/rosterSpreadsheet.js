...
export async function unzipEntries(buffer) {
  ...
  for (let index = 0; index < totalEntries; index += 1) {
    ...
    const name = textDecoder.decode(bytes.slice(cursor + 46, cursor + 46 + nameLength)).replace(/^\/+/, '');
    ...
    if (uncompressedSize && data.byteLength !== uncompressedSize) throw new Error('xlsx_zip_size_mismatch');
...
export function parseSharedStrings(xml = '') {
  ...
  for (const item of String(xml).matchAll(/<si\b[^>]*>([\s\S]*?)<\/si>/gi)) {
    const text = [...item[1].matchAll(/<t\b[^>]*>([\s\S]*?)<\/t>/gi)].map((match) => decodeXml(match[1])).join('');
    values.push(text);
  }
  return values;
}
...
export function parseWorksheet(xml = '', sharedStrings = []) {
  ...
  for (const rowMatch of String(xml).matchAll(/<row\b[^>]*>([\s\S]*?)<\/row>/gi)) {
    ...
    for (const cellMatch of rowMatch[1].matchAll(/<c\b([^>]*)>([\s\S]*?)<\/c>/gi)) {
      ...
      const raw = body.match(/<v\b[^>]*>([\s\S]*?)<\/v>/i)?.[1] ?? [...inline.matchAll(/<t\b[^>]*>([\s\S]*?)<\/t>/gi)].map((match) => match[1]).join('');
...
export function resolveFirstWorksheet(entries) {
  ...
  if (workbookBytes && relationsBytes) {
    ...
    if (relationId) {
      const relationRegex = new RegExp(`<Relationship\\b[^>]*\\bId="${relationId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"[^>]*\\bTarget="([^"]+)"`, 'i');
...
export async function parseRosterXlsxBuffer(buffer) {
  const bytes = buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer);
  if (!bytes.byteLength || bytes.byteLength > MAX_SPREADSHEET_BYTES) throw new Error('xlsx_size_invalid');
...
export async function parseRosterFile(file) {
  ...
  if (name.endsWith('.csv') || type.includes('csv') || type === 'text/plain') return parseRosterCsv(await file.text());
  if (name.endsWith('.xlsx') || type.includes('spreadsheetml')) return parseRosterXlsxBuffer(await file.arrayBuffer());
  throw new Error('roster_file_type_unsupported');
}

export const ROSTER_FILE_ACCEPT = '.csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
