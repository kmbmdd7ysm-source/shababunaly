      ...
      <main className="tw-page">
        ...
        <section id="team-quote" className="tw-quote team-quote-grid">
          ...
          <form onSubmit={(e) => { void submit(e); }}>
            <div className="field-row"><label className="field" data-field="name"><span className="field__label">{pick({ en: 'Full name', ar: 'الاسم الكامل' })}</span><div className="field__control"><input required value={form.name} onChange={(e)=>setForm({...form,name:e.target.value})} autoComplete="name" /></div></label><label className="field" data-field="organization"><span className="field__label">{pick({ en: 'Team / Organization', ar: 'الفريق / المؤسسة' })}</span><div className="field__control"><input required value={form.organization} onChange={(e)=>setForm({...form,organization:e.target.value})} autoComplete="organization" /></div></label></div>
            <div className="field-row"><label className="field" data-field="email"><span className="field__label">Email</span><div className="field__control field__control--latin"><input required type="email" value={form.email} onChange={(e)=>setForm({...form,email:e.target.value})} autoComplete="email" /></div></label><label className="field" data-field="phone"><span className="field__label">{pick({ en: 'Phone / WhatsApp', ar: 'الهاتف / واتساب' })}</span><div className="field__control field__control--latin"><input value={form.phone} onChange={(e)=>setForm({...form,phone:e.target.value})} autoComplete="tel" /></div></label></div>
            <div className="field-row"><label className="field" data-field="type"><span className="field__label">{pick({ en: 'Organization type', ar: 'نوع المؤسسة' })}</span><div className="field__control field__control--select"><select value={form.type} onChange={(e)=>setForm({...form,type:e.target.value})}><option value="club">Club</option><option value="academy">Academy</option><option value="federation">Federation</option><option value="school_university">School / University</option><option value="wholesale">Wholesale</option></select></div></label><label className="field" data-field="country"><span className="field__label">{pick({ en: 'Country', ar: 'الدولة' })}</span><div className="field__control"><CountrySelect value={form.country} onChange={(country)=>setForm({...form,country})} /></div></label></div>
            <div className="field-row"><label className="field" data-field="quantity"><span className="field__label">{pick({ en: 'Estimated quantity', ar: 'الكمية التقديرية' })}</span><div className="field__control field__control--latin"><input required inputMode="numeric" value={form.quantity} onChange={(e)=>setForm({...form,quantity:e.target.value.replace(/\D/g,'')})} /></div></label><label className="field" data-field="deadline"><span className="field__label">{pick({ en: 'Needed by', ar: 'الموعد المطلوب' })}</span><div className="field__control field__control--date field__control--latin"><input type="date" value={form.deadline} onChange={(e)=>setForm({...form,deadline:e.target.value})} /></div></label></div>
            <label className="field" data-field="message"><span className="field__label">{pick({ en: 'What do you need?', ar: 'شنو تحتاج؟' })}</span><div className="field__control field__control--textarea"><textarea required rows={5} value={form.needs} onChange={(e)=>setForm({...form,needs:e.target.value})} placeholder={pick({ en: 'Example: 24 home/away uniforms, sizes ready, logo available.', ar: 'مثال: 24 طقم أساسي واحتياطي، المقاسات جاهزة، والشعار موجود.' })} /></div></label>
