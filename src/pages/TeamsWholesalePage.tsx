      ...
      <main className="tw-page">
        ...
        <section id="team-quote" className="tw-quote team-quote-grid">
          ...
          <form onSubmit={(e) => { void submit(e); }}>
            <div className="field-row"><label className="field" data-field="name"><span>{pick({ en: 'Full name', ar: 'الاسم الكامل' })}</span><input required value={form.name} onChange={(e)=>setForm({...form,name:e.target.value})} autoComplete="name" /></label><label className="field" data-field="organization"><span>{pick({ en: 'Team / Organization', ar: 'الفريق / المؤسسة' })}</span><input required value={form.organization} onChange={(e)=>setForm({...form,organization:e.target.value})} autoComplete="organization" /></label></div>
            <div className="field-row"><label className="field" data-field="email"><span>Email</span><input required type="email" value={form.email} onChange={(e)=>setForm({...form,email:e.target.value})} autoComplete="email" /></label><label className="field" data-field="phone"><span>{pick({ en: 'Phone / WhatsApp', ar: 'الهاتف / واتساب' })}</span><input value={form.phone} onChange={(e)=>setForm({...form,phone:e.target.value})} autoComplete="tel" /></label></div>
            <div className="field-row"><label className="field" data-field="type"><span>{pick({ en: 'Organization type', ar: 'نوع المؤسسة' })}</span><select value={form.type} onChange={(e)=>setForm({...form,type:e.target.value})}><option value="club">Club</option><option value="academy">Academy</option><option value="federation">Federation</option><option value="school_university">School / University</option><option value="wholesale">Wholesale</option></select></label><label className="field" data-field="country"><span>{pick({ en: 'Country', ar: 'الدولة' })}</span><CountrySelect value={form.country} onChange={(country)=>setForm({...form,country})} /></label></div>
            <div className="field-row"><label className="field" data-field="quantity"><span>{pick({ en: 'Estimated quantity', ar: 'الكمية التقديرية' })}</span><input required inputMode="numeric" value={form.quantity} onChange={(e)=>setForm({...form,quantity:e.target.value.replace(/\D/g,'')})} /></label><label className="field" data-field="deadline"><span>{pick({ en: 'Needed by', ar: 'الموعد المطلوب' })}</span><input type="date" value={form.deadline} onChange={(e)=>setForm({...form,deadline:e.target.value})} /></label></div>
            <label className="field" data-field="message"><span>{pick({ en: 'What do you need?', ar: 'شنو تحتاج؟' })}</span><textarea required rows={5} value={form.needs} onChange={(e)=>setForm({...form,needs:e.target.value})} placeholder={pick({ en: 'Example: 24 home/away uniforms, sizes ready, logo available.', ar: 'مثال: 24 طقم أساسي واحتياطي، المقاسات جاهزة، والشعار موجود.' })} /></label>
