...
const links = ['dashboard','orders','payments','b2b','shipping','catalog','inventory','media','security','users','settings'];
export default function OperationsPage() {
  const auth = useAuth(); const { pick } = useLanguage();
  ...
  return <div className="operations-shell" data-staff-role={getStaffRole(auth.user)}>
    <nav className="operations-module-nav" aria-label={pick({ en: 'Operations modules', ar: 'أقسام العمليات' })}>
      {links.map((name) => <NavLink key={name} to={`/operations/${name}`}>{name === 'b2b' ? 'B2B' : name[0].toUpperCase()+name.slice(1)}</NavLink>)}
    </nav>
    <Suspense fallback={<p role="status">{pick({ en: 'Loading operations module…', ar: 'جاري تحميل قسم العمليات…' })}</p>}>
      <Routes>
