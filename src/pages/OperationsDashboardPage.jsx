  ...
  const filteredCatalog = useMemo(() => {
    ...
    return state.catalog
      .filter(
        (row) =>
          !q ||
          [row.product_name, row.sku, row.variant_id, row.canonical_slug].some((value) =>
            String(value || '')
              .toLowerCase()
              .includes(q),
          ),
        ...
        <div>
          <div className="operations-toolbar">
            <Stat
              ...
              value={pendingShipping.length}
            />
            <Stat
              label={pick({ en: 'Active orders', ar: 'الطلبات النشطة' })}
              value={activeOrders.length}
            />
            <Stat
              label={pick({ en: 'Active quotes', ar: 'عروض الأسعار النشطة' })}
              value={state.quotes.length}
            />
            <Stat
              label={pick({ en: 'Open returns', ar: 'طلبات الإرجاع المفتوحة' })}
              value={state.returns.length}
            />
            <Stat
              label={pick({ en: 'Special requests', ar: 'الطلبات الخاصة' })}
              value={state.specialRequests.length}
            />
            <Stat
              label={pick({ en: 'Catalog variants', ar: 'خيارات المنتجات' })}
            ...
            <form
              onSubmit={(event) => {
                ...
                run(
                  ...
                  pick({
                    en: 'Exchange rate saved and notification queued.',
                    ar: 'تم حفظ سعر الصرف وإضافة الإشعار.',
                  }),
                );
              }}
              className="operations-rate"
            >
              <label>
                <span>USD → LYD</span>
                <input
              ...
              <button className="btn-secondary" disabled={saving === 'rate'}>
                {pick({ en: 'Save Rate', ar: 'حفظ السعر' })}
              </button>
            </form>
          </div>
          {(state.error || notice) && (
          ...
          {state.loading ? (
            <p role="status">{pick({ en: 'Loading operations…', ar: 'جاري تحميل العمليات…' })}</p>
          ) : (
            <>
              <section className="operations-section">
                <h2>{pick({ en: 'Country shipping rates', ar: 'أسعار الشحن حسب الدولة' })}</h2>
                <p>
                  {pick({
                    en: 'Retail checkout uses these prices automatically. Countries without an active price stay pending for a manual quote. Custom, wholesale and large equipment always remain quote-based.',
                    ar: 'يستخدم الدفع هذه الأسعار تلقائيًا للطلبات العادية. الدول من دون سعر نشط تبقى معلقة للتسعير، بينما التصميم والجملة والمعدات الكبيرة تحتاج دائمًا عرض شحن مستقلًا.',
                  })}
                </p>
                <ShippingRatesManager
              ...
              <section className="operations-section">
                <h2>{pick({ en: 'Homepage hero media', ar: 'وسائط هيرو الصفحة الرئيسية' })}</h2>
                <HeroContentManager
              ...
              {getStaffRole(auth.user) === 'super_admin' && (
                <section className="operations-section">
                  <h2>{pick({ en: 'Staff access', ar: 'صلاحيات الموظفين' })}</h2>
                  <p>
                    {pick({
                      en: 'Roles are stored in protected Supabase app metadata. Customers cannot grant themselves access.',
                      ar: 'تُحفظ الصلاحيات داخل بيانات Supabase المحمية، ولا يستطيع العميل منح نفسه صلاحية.',
                    })}
                  </p>
                  <StaffAccessManager
                    state={adminUsers}
              ...
              <section className="operations-section">
                <h2>{pick({ en: 'Enterprise master data', ar: 'البيانات الرئيسية للمؤسسة' })}</h2>
                <p>
                  {pick({
                    en: 'Manage brands, categories, warehouses, suppliers and carriers from protected database records.',
              ...
              <section className="operations-section">
                ...
                <div className="operations-toolbar">
                  <Stat
                    label={pick({ en: 'Invoices', ar: 'الفواتير' })}
                    value={state.invoices.length}
                  />
                  <Stat
                    label={pick({ en: 'Purchase orders', ar: 'أوامر الشراء' })}
                    value={state.purchaseOrders.length}
                  />
                  <Stat
                    label={pick({ en: 'Shipments', ar: 'الشحنات' })}
                    value={state.shipments.length}
                  />
                  <Stat
                    label={pick({ en: 'Contracts', ar: 'العقود' })}
                    value={state.contracts.length}
                  />
                  <Stat
                    label={pick({ en: 'Payment proofs', ar: 'إثباتات الدفع' })}
                    value={state.paymentProofs.length}
                  />
                  <Stat
                    label={pick({ en: 'Reorders', ar: 'إعادات الطلب' })}
                    value={state.reorders.length}
                  />
                  <Stat
                    label={pick({ en: 'Team lockers', ar: 'متاجر الفرق' })}
                    value={state.lockers.length}
                  />
                  <Stat
                    label={pick({ en: 'Failed notifications', ar: 'الإشعارات الفاشلة' })}
                    value={
                      state.notifications.filter((row) => row.delivery_status === 'failed').length
                    }
                  />
                  <Stat
                ...
                <details>
                  <summary>
                    {pick({ en: 'Recent audit records', ar: 'أحدث سجلات التدقيق' })}
                  </summary>
                  <ul>
              ...
              <section className="operations-section">
                ...
                <div className="operations-table-wrap">
                  <table className="operations-table">
                    <thead>
                      <tr>
                        <th>{pick({ en: 'Order', ar: 'الطلب' })}</th>
                        <th>{pick({ en: 'Destination', ar: 'الوجهة' })}</th>
                        <th>{pick({ en: 'Products', ar: 'المنتجات' })}</th>
                        <th>{pick({ en: 'Shipping USD', ar: 'الشحن بالدولار' })}</th>
              ...
              <section className="operations-section">
                ...
                <div className="operations-card-grid">
                  ...
                  {!state.specialRequests.length && (
                    <p>
                      {pick({ en: 'No open special requests.', ar: 'لا توجد طلبات خاصة مفتوحة.' })}
                    </p>
                  )}
                </div>
              </section>
              <section className="operations-section">
                <h2>{pick({ en: 'Order workflow & payments', ar: 'مراحل الطلبات والمدفوعات' })}</h2>
                <div className="operations-card-grid">
              ...
              <section className="operations-section">
                <h2>{pick({ en: 'Returns & refunds', ar: 'الإرجاع واسترداد المبالغ' })}</h2>
                <div className="operations-card-grid">
                  ...
                  {!state.returns.length && (
                    <p>
                      {pick({ en: 'No open return requests.', ar: 'لا توجد طلبات إرجاع مفتوحة.' })}
                    </p>
                  )}
                </div>
              </section>
              ...
              <section className="operations-section">
                <h2>{pick({ en: 'Design proof approvals', ar: 'اعتمادات بروفات التصميم' })}</h2>
                <div className="operations-card-grid">
                  ...
                  {!state.designs.length && (
                    <p>
                      {pick({ en: 'No active design proofs.', ar: 'لا توجد بروفات تصميم نشطة.' })}
                    </p>
                  )}
                </div>
              </section>
              <section className="operations-section">
                <h2>
                  {pick({ en: 'Product content & merchandising', ar: 'محتوى المنتجات والعرض' })}
                </h2>
              ...
              <section className="operations-section">
                <div className="section-heading-row">
                  <h2>
                    {pick({ en: 'Catalog variants & inventory', ar: 'خيارات المنتجات والمخزون' })}
                  </h2>
                  <input
                    className="operations-search"
                    value={catalogQuery}
                    onChange={(event) => setCatalogQuery(event.target.value)}
                    placeholder={pick({ en: 'Search SKU or product', ar: 'ابحث بالمنتج أو SKU' })}
                  />
                </div>
                <div className="operations-table-wrap">
                  <table className="operations-table catalog-operations-table">
                    <thead>
                      <tr>
                        <th>SKU</th>
                        <th>{pick({ en: 'Product', ar: 'المنتج' })}</th>
                        <th>{pick({ en: 'Price', ar: 'السعر' })}</th>
                        <th>{pick({ en: 'Wholesale', ar: 'الجملة' })}</th>
                        <th>{pick({ en: 'Inventory', ar: 'المخزون' })}</th>
                        <th>{pick({ en: 'Ready', ar: 'فوري' })}</th>
                        <th>{pick({ en: 'Active', ar: 'نشط' })}</th>
                        <th>{pick({ en: 'Save', ar: 'حفظ' })}</th>
                      </tr>
                    </thead>
                    <tbody>
