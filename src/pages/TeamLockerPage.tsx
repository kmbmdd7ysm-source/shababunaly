...
export default function TeamLockerPage(): ReactElement | null {
  ...
  const [state, setState] = useState<{
    ...
    error: string;
  }>({ loading: true, store: null, rows: [], error: '' });
  useEffect(() => {
    let active = true;
  ...
  const products = useMemo(
    () =>
      state.rows
        .map((row) => catalog.getProductById(String(row.product_id || '')))
  ...
  const storeName = String(state.store?.name || '');
  const storeDescription = String(state.store?.description || '');

  return (
    <>
      <Seo title={storeName || 'Team Locker'} path={`/team-locker/${slug}`} noindex />
