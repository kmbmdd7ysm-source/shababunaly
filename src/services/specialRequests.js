...
export function validateSpecialRequestFiles(productImage, additionalFiles = []) {
  ...
  for (const { file, role } of entries) {
    if (!(file instanceof File) || !file.size || file.size > MAX_FILE_BYTES) throw new Error('invalid_file_size');
    total += file.size;
    if (total > MAX_TOTAL_BYTES) throw new Error('files_too_large');
    const allowedExtensions = ACCEPTED.get(file.type);
    if (!allowedExtensions?.includes(extension(file.name))) throw new Error('unsupported_file_type');
    if (role === 'product_image' && !file.type.startsWith('image/')) throw new Error('product_image_must_be_image');
  }
  return entries;
}

export async function submitSpecialRequest({ payload, productImage = null, additionalFiles = [], turnstileToken, accessToken = '' }) {
  ...
  const response = await fetch('/api/special-request', {
    ...
    body: JSON.stringify({ ...payload, files, turnstileToken, idempotencyKey: crypto.randomUUID() }),
  });
...
export async function getMySpecialRequests() {
  ...
  const { data, error } = await client.from('special_requests').select('*').order('created_at', { ascending: false });
