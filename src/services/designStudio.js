    ...
    layers: [
      {
        ...
        width: 46,
        rotation: 0,
        color: design.secondary || '#ffffff',
        font: design.font || 'block',
        visible: true,
        locked: false,
        zIndex: 20,
      },
  ...
  return {
    ...
    layers: layers.slice(0, 80).map((layer, index) => ({
      ...
      fontLicenseStatus: String(
        layer.fontLicenseStatus || (layer.type === 'logo' ? 'not_applicable' : 'built_in_licensed'),
      ).slice(0, 60),
    })),
...
export function addDesignLayer(studio, partial = {}, design = {}) {
  ...
  const sameView = current.layers.filter(
    (layer) => layer.view === (partial.view || current.activeView),
  );
  const layer = {
    ...
    content: String(partial.content || (partial.type === 'number' ? '00' : 'TEXT')).slice(
      0,
      partial.type === 'logo' ? 2_000_000 : 120,
    ),
    ...
    color: /^#[0-9a-f]{6}$/i.test(String(partial.color || ''))
      ? partial.color
      : design.secondary || '#ffffff',
    font: ['block', 'condensed', 'modern'].includes(partial.font)
      ? partial.font
      : design.font || 'block',
    visible: true,
    locked: false,
    ...
    pixelWidth: partial.pixelWidth || 0,
    pixelHeight: partial.pixelHeight || 0,
    vectorSourceValidated: Boolean(partial.vectorSourceValidated),
    fontLicenseStatus:
      partial.fontLicenseStatus ||
      (partial.type === 'logo' ? 'not_applicable' : 'built_in_licensed'),
  };
  ...
  return normalizeStudio(
    {
      ...
      layers: [
        ...current.layers,
        {
          ...
          zIndex: Math.max(...current.layers.map((layer) => layer.zIndex)) + 1,
        },
      ],
    },
    design,
...
export function addDesignComment(studio, comment, design = {}) {
  ...
  const next = {
    id: uid('comment'),
    view: VIEWS.has(comment.view) ? comment.view : current.activeView,
...
async function designShareRequest(token, payload = null) {
  ...
  if (!response.ok || !data?.ok)
    throw Object.assign(new Error(data?.error || 'design_share_unavailable'), {
...
export async function loadSharedDesign(token) {
  ...
  const design = {
    ...(data.designData || {}),
    productType: data.productType || data.designData?.productType,
  };
