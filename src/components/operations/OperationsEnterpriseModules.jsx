...
export function InventoryCsvManager({ state, pick, saving, run }) {
  ...
  const applyBatch = async () => {
    ...
    return applyInventoryImport({ sourceName: fileName, rows, batchId: preview.batchId });
  };
  const exportCsv = () => {
    const blob = new Blob([createInventoryCsv(state.warehouseInventory)], {
    ...
    <div className="enterprise-panel-stack inventory-csv-manager">
      <div className="section-heading-row">
        ...
        <button
          ...
          className="btn-secondary compact"
          onClick={exportCsv}
          disabled={!state.warehouseInventory.length}
        >
        ...
        {preview && (
          <div className="workspace-list">
            {(preview.errors || []).map((row) => (
              <article key={`error-${row.row}`}>
                <div>
                  <strong>{pick({ en: `Row ${row.row}`, ar: `الصف ${row.row}` })}</strong>
                  <p>
                    {row.sku} · {row.error}
                  </p>
                </div>
            ...
            {(preview.preview || []).slice(0, 50).map((row) => (
              <article key={`${row.warehouseId}-${row.variantId}`}>
                <div>
                  <strong>{row.sku}</strong>
                  <p>
                    {row.warehouseCode}: {row.beforeOnHand} → {row.afterOnHand}
                  </p>
                </div>
                <small>
                  {pick({ en: 'Reserved', ar: 'محجوز' })}: {row.reserved}
                </small>
              </article>
            ))}
          </div>
      ...
      <details>
        ...
        <div className="workspace-list">
          {state.inventoryImports.map((batch) => (
            <article key={batch.id}>
              <div>
                <span className="workspace-status-dot" data-status={batch.status} />
                <div>
                  <h3>{batch.source_name || batch.id}</h3>
                  <p>
                    {batch.status} · {batch.row_count} rows · {batch.error_count} errors
                  </p>
                </div>
              </div>
              {batch.status === 'applied' && (
                <button
                  type="button"
                  className="btn-secondary compact"
                  disabled={saving === `rollback-${batch.id}`}
                  onClick={() =>
                    run(
                      `rollback-${batch.id}`,
                      () => rollbackInventoryImport(batch.id),
                      pick({
