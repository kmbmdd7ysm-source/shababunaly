    ...
    <section className="operations-subsection">
      ...
      <div className="operations-card-grid">
        {failed.slice(0, 25).map((row) => (
          <article className="operations-card" key={row.id}>
            <div>
            ...
            <p>{row.last_error || '—'}</p>
            <small>
              {pick({ en: 'Attempts', ar: 'المحاولات' })}: {row.attempts}
            </small>
            <button
              type="button"
              className="btn-secondary compact"
              disabled={saving === `notification-${row.id}`}
              onClick={() =>
                run(
                  `notification-${row.id}`,
                  () => retryCommerceNotification(row.id),
                  pick({
        ...
        {alerts.slice(0, 25).map((row) => (
          <article className="operations-card" key={row.id}>
            ...
            <button
              type="button"
              className="btn-secondary compact"
              disabled={saving === `security-${row.id}`}
              onClick={() =>
                run(
                  `security-${row.id}`,
                  () => resolveSecurityEvent(row.id, true),
