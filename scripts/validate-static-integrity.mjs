...
for (const file of walk('.').filter((name) => name.endsWith('.json') && !name.includes('node_modules') && !name.includes('reports'))) {
...
const importTargets = [...app.matchAll(/import\(['"](\.\.?\/[^'"]+)['"]\)/g)].map((match) => match[1]);
for (const target of importTargets) {
  const base = path.resolve('src', target.replace(/^\.\//, ''));
  if (![base, `${base}.js`, `${base}.jsx`, path.join(base, 'index.js'), path.join(base, 'index.jsx')].some(existsSync)) {
    failures.push(`App lazy import target is missing: ${target}`);
  }
}

if (failures.length) {
  console.error(`Static-integrity validation failed:\n${failures.map((item) => `- ${item}`).join('\n')}`);
  process.exit(1);
}
console.info('Static integrity passed: JSON, CSS, HTML shell and lazy-route targets are structurally valid.');
