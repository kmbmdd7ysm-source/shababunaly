...
async function walk(directory) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) await walk(absolute);
      ...
      if (!['.js', '.css', '.html', '.json', '.svg', '.webp', '.png', '.jpg', '.jpeg', '.woff2'].includes(extension)) continue;
...
const htmlRows = rows.map((row) => `<tr><td>${row.file}</td><td>${row.type}</td><td>${format(row.bytes)}</td><td>${format(row.gzipBytes)}</td><td>${format(row.brotliBytes)}</td></tr>`).join('');
...
await writeFile('reports/bundle-stats.json', `${JSON.stringify({ generatedAt: new Date().toISOString(), totals, assets: rows }, null, 2)}\n`);
