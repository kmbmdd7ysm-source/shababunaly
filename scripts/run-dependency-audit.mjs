...
try {
  ...
  parsed = {
    error: { message: 'invalid_npm_audit_json' },
    stdout: run.stdout || '',
    stderr: run.stderr || '',
  };
}
...
const auditCompleted =
  !run.error && run.status === 0 && Boolean(parsed.metadata?.vulnerabilities) && !parsed.error;
const report = {
...
if (!auditCompleted || blocking > 0) {
  const reason =
    ...
    parsed.message ||
    (blocking > 0
      ? `Dependency audit found ${blocking} high/critical vulnerabilities.`
      : `npm audit did not complete successfully (exit ${run.status}).`);
