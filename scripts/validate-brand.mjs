...
const configFiles = ['src/config.ts', 'src/config/brand.ts'];
...
if (!html.includes('SHABABUNA') || !html.includes('BUILT DIFFERENT')) {
  console.error('BROKEN CONFIGURATION: production brand metadata missing');
  errors += 1;
}
 ...
 * Quarantine gate.
 *
 * The artwork under public/brand/quarantine/ contains a reproduction of the NBA
 * Logoman. It was previously wired in as the Shababuna logo — in the favicon,
 * the app icon, the Open Graph card and the Organization.logo field of the
 * site's structured data. It must never be referenced again, and it must never
 * be served: `dist` is built from `public`, so a file left in the wrong place
 * ships. This fails the build rather than trusting anyone to remember.
 */
const QUARANTINED = [
  'shababuna-mark-black.png',
  'shababuna-mark-white.png',
  'shababuna-full-en-black.png',
  'shababuna-full-en-white.png',
  'shababuna-full-ar-black.png',
  'shababuna-full-ar-white.png',
  'favicon-legacy.svg',
  'favicon-legacy.png',
  'shababuna-social-legacy.png',
];
for (const url of referenced) {
  const name = url.split('/').pop();
  if (QUARANTINED.includes(name) || url.includes('/quarantine/')) {
    console.error('QUARANTINED BRAND ASSET REFERENCED:', url);
    console.error(
      '  This artwork reproduces a third-party trademark. See public/brand/quarantine/README.md.',
    );
    errors += 1;
  }
}
for (const name of QUARANTINED) {
  if (existsSync(path.join('public/brand', name)) || existsSync(path.join('public', name))) {
    console.error('QUARANTINED BRAND ASSET IS IN A SHIPPED DIRECTORY:', name);
    errors += 1;
  }
}
