...
const bilingual = (value) =>
  value && typeof value === 'object' && nonEmpty(value.en) && nonEmpty(value.ar);
...
unique(
  catalogProducts.flatMap((product) => product.variants || []).map((variant) => variant.sku),
  'catalog variant sku',
);
...
for (const category of categories) {
  ...
  unique(
    ...
    `subcategory slug in ${category.slug}`,
  );
  for (const sub of category.subcategories || [])
    if (!bilingual(sub.name)) err(`subcategory ${sub.slug}: bilingual name is required`);
}

for (const product of catalogProducts) {
  const label = `${product.id}/${product.slug}`;
  if (!/^[a-z0-9-]+$/.test(product.slug || '')) err(`${label}: invalid slug`);
  if (!bilingual(product.name) || product.name.en !== product.name.ar)
    err(`${label}: product name must stay English in both languages`);
  if (!bilingual(product.description))
    err(`${label}: natural English and Arabic descriptions are required`);
  ...
  if (
    ...
    product.subcategory &&
    !subcategorySlugs.get(product.category)?.has(product.subcategory)
  )
    err(`${label}: unknown subcategory ${product.category}/${product.subcategory}`);
  if (!Number.isFinite(product.price) || product.price < 0)
    err(`${label}: invalid USD retail price`);
  if (isProductVisible(product) && getProductPublishIssues(product).length)
    err(
      `${label}: visible product has publish blockers: ${getProductPublishIssues(product).join(', ')}`,
    );
  if (product.currency !== 'USD') err(`${label}: base currency must be USD`);
  if (!Array.isArray(product.variants) || product.variants.length === 0)
    err(`${label}: at least one variant is required`);
  if (!Array.isArray(product.storefronts) || product.storefronts.length === 0)
    err(`${label}: storefront assignment is required`);
  if (product.wholesaleAvailable) {
    if (!Number.isInteger(product.wholesaleMin) || product.wholesaleMin < 1)
      err(`${label}: wholesale minimum must be a positive integer`);
    if (
      !Number.isFinite(product.wholesalePrice) ||
      product.wholesalePrice < 0 ||
      product.wholesalePrice >= product.price
    )
      err(`${label}: wholesale price must be lower than retail`);
  }
  if (product.customizable) {
    const expected =
      product.category === 'basketballs' ? 6 : product.category === 'equipment' ? 1 : 10;
    if (product.minimumOrder < expected && !product.legacyLha)
      err(`${label}: custom minimum must be at least ${expected}`);
  }
  if (product.madeInUSA && (product.category !== 'clothing' || product.brand !== 'Shababuna'))
    err(`${label}: Made in USA may only be applied to Shababuna apparel`);
  if (product.largeEquipment && product.category !== 'equipment')
    err(`${label}: largeEquipment is restricted to equipment`);
  for (const variant of product.variants || []) {
    if (!nonEmpty(variant.sku)) err(`${label}: variant SKU is required`);
    if (!Number.isInteger(variant.stock) || variant.stock < 0)
      err(`${label}: variant stock must be a non-negative integer`);
  }
}
...
if (lha.length !== sourceLhaProducts.length)
  err(
    `LHA store must contain all ${sourceLhaProducts.length} source products; received ${lha.length}`,
  );
for (const source of sourceLhaProducts) {
  const copied = lha.find(
    (product) => product.legacySourceId === source.id || product.slug === `lha-${source.slug}`,
  );
  if (!copied) err(`LHA product not copied: ${source.slug}`);
  else if (copied.price !== source.price)
...
const draftCustomCount = catalogProducts.filter(
  (product) => product.customizable && !isProductVisible(product),
).length;
if (!readyCount)
  console.info(
