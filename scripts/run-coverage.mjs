...
const reportName = process.argv.includes('--critical') ? 'critical-coverage.txt' : 'node-coverage.txt';
const args = [
...
if (run.status !== 0 || !summary || summary.slice(1).some((value) => Number(value) !== 100)) {
  console.error('Coverage gate failed: runtime lines, branches and functions in the declared Node/API scope must all equal 100.00%.');
  process.exit(run.status || 1);
}
console.log(`Verified Node/API runtime coverage: lines ${summary[1]}%, branches ${summary[2]}%, functions ${summary[3]}%.`);
