...
//   1. parse the route table out of src/App.tsx
...
for (const { path, component } of routes) {
  ...
  const numstat = file ? git(['diff', '--numstat', BASELINE, '--', file]).trim() : '';
...
const md = [
  ...
  '',
  `Generated by \`scripts/generate-route-ledger.mjs\` from the real router in \`src/App.tsx\`,`,
  `diffed against \`${BASELINE}\` (working tree, so uncommitted work counts) on ${report.generatedAt.slice(0, 10)}.`,
  '',
