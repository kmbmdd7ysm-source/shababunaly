...
const validReviewDate = (value) => { const parsed=Date.parse(String(value||'')); return Number.isFinite(parsed) && parsed <= Date.now(); };
...
const suspiciousArabic = arabicKeys.filter((key) => {
  ...
  return !/[\u0600-\u06ff]/.test(value) && !/^(USD|LYD|SKU|PDF|MFA|CTA|B2B|AVIF|WebP|Email|WhatsApp)$/i.test(value);
});
const sections = Object.entries(manifest.sections || {});
const evaluatedSections = sections.map(([name, value]) => {
  const evidence = Boolean(
    value?.status === 'approved' && String(value?.reviewer || '').trim() && String(value?.reviewerRole || '').trim() &&
    validReviewDate(value?.reviewedAt) && value?.sourceSha256 === translationSourceSha256
  );
  return { name, status: value?.status || 'missing', approved: evidence, stale: value?.status === 'approved' && value?.sourceSha256 !== translationSourceSha256 };
});
...
const structuralPass = missingArabic.length === 0 && missingEnglish.length === 0 && emptyArabic.length === 0;
const productionReady = structuralPass && allSectionsApproved && manifestSourceMatches && sourceCommitPresent;
const report = {
  ...
  sourceCommitPresent,
  counts: { englishKeys: englishKeys.length, arabicKeys: arabicKeys.length, approvedSections: approvedSections.length, totalSections: sections.length },
  structuralPass,
  productionReady,
  missingArabic,
  missingEnglish,
...
if (!structuralPass) {
  console.error(`Arabic localization structure failed: missing AR ${missingArabic.length}, missing EN ${missingEnglish.length}, empty AR ${emptyArabic.length}.`);
  process.exit(1);
}
if (requireApproval && !productionReady) {
  console.error('Arabic production review blocked: each section needs a named reviewer, role, date and the exact current translation SHA-256; the manifest also needs the current source hash and commit.');
  process.exit(1);
}
console.info(`Arabic localization structure passed (${arabicKeys.length} keys). Human review: ${productionReady ? 'approved' : `${approvedSections.length}/${sections.length} current-hash sections approved`}.`);
