...
const evidence = { status: 'passed', generatedAt: new Date().toISOString(), commitSha: process.env.GITHUB_SHA || null, runId: process.env.GITHUB_RUN_ID || null, checks: [] };
mkdirSync('reports/quality', { recursive: true });
for (const [name, command] of commands) {
  const [bin, ...args] = command;
  const run = spawnSync(bin, args, { encoding: 'utf8', shell: process.platform === 'win32', maxBuffer: 40*1024*1024 });
  const output = `${run.stdout || ''}${run.stderr || ''}`;
  writeFileSync(`reports/quality/${name}.log`, output);
  evidence.checks.push({ name, status: run.status === 0 ? 'passed' : 'failed', exitCode: run.status ?? 1 });
