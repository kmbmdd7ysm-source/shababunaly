...
// The order-first invariant is semantic, not formatting-dependent: a trusted
// pending order must be persisted and its confirmation returned before the
// browser is allowed to request a provider checkout session.
const saveMatch = /(?:const|let)\s+confirmation\s*=\s*\(?await\s+savePendingOrder\s*\(/m.exec(checkout);
const saveIndex = saveMatch?.index ?? -1;
const sessionIndex = checkout.indexOf('createCheckoutSession({', Math.max(0, saveIndex));
if (saveIndex < 0 || sessionIndex < 0 || sessionIndex < saveIndex)
