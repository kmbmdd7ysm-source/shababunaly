...
const mime = {
  ...
  '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.woff2': 'font/woff2', '.ico': 'image/x-icon',
};
...
const handler = async (request, response) => {
  try {
    ...
    catch { candidate = join(activeDir, 'index.html'); }
    const body = await readFile(candidate);
    response.statusCode = 200;
    response.setHeader('content-type', mime[extname(candidate)] || 'application/octet-stream');
    response.setHeader('cache-control', pathname === '/sw.js' || candidate.endsWith('index.html') ? 'no-store' : 'public, max-age=31536000, immutable');
    response.end(body);
  } catch (error) {
...
const result = { status: 'failed', generatedAt: new Date().toISOString(), versions: versions.map((v) => v.id), checks: [], error: null };
let server;
let browser;
try {
  ...
  await page.waitForFunction(async (id) => (await caches.keys()).some((key) => key.includes(id)), versions[0].id);
  const firstKeys = await page.evaluate(() => caches.keys());
  result.checks.push({ name: 'version_a_installed', passed: firstKeys.some((key) => key.includes(versions[0].id)), cacheKeys: firstKeys });
  ...
  await page.reload({ waitUntil: 'networkidle' });
  await page.waitForFunction(async ([oldId, newId]) => {
  ...
  result.checks.push({ name: 'old_caches_removed', passed: secondKeys.every((key) => !key.includes(versions[0].id)), cacheKeys: secondKeys });
  result.checks.push({ name: 'version_b_active', passed: secondKeys.some((key) => key.includes(versions[1].id)), cacheKeys: secondKeys });
  const sensitive = await page.evaluate(async () => {
    const matches = [];
    for (const key of await caches.keys()) {
      const cache = await caches.open(key);
      for (const request of await cache.keys()) if (/\/(?:api|account|checkout|operations|orders)(?:\/|\?|$)/.test(new URL(request.url).pathname)) matches.push(request.url);
    }
    return matches;
  });
  result.checks.push({ name: 'sensitive_routes_not_cached', passed: sensitive.length === 0, matches: sensitive });
...
console.info('PWA upgrade verified: the new build activates, old caches are removed and sensitive routes remain uncached.');
