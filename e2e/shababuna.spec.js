...
test('special request validates URL-or-image and supports RTL', async ({ page }) => {
  ...
  await page
    .getByLabel(/Description|الوصف/i)
    .fill('A real basketball product requested for sourcing.');
...
test('special request URL submission handles a verified API response', async ({ page }) => {
  await page.route('**/api/special-request', async (route) =>
    route.fulfill({
      ...
      body: JSON.stringify({ ok: true, request: { request_number: 'SR-20260801-0000001' } }),
    }),
  );
  await page.goto('/special-request');
  await page.getByLabel(/Customer name|اسم العميل/i).fill('Test Customer');
  ...
  await page
    .getByLabel(/Description|الوصف/i)
    .fill('A verified basketball product sourcing request.');
  await page.getByLabel(/Desired quantity|الكمية المطلوبة/i).fill('2');
  await page.getByRole('checkbox').check();
  await page.getByRole('button', { name: /Submit Special Request|إرسال الطلب الخاص/i }).click();
...
test('protected staff and team-locker routes never expose private data to guests', async ({
  page,
}) => {
  await page.goto('/operations');
  await expect(page).toHaveURL(/\/account/);
...
test('all public routes render without runtime, console or network-fatal errors', async ({
  page,
}) => {
  const failures = [];
  ...
  for (const route of routes) {
    ...
    await expect(page.locator('body')).not.toContainText(
      /ReferenceError|Application error|is not defined/,
    );
  }
  expect(failures).toEqual([]);
});

test('custom production studio exposes production-grade controls and XLSX import', async ({
  page,
}) => {
  await page.goto('/customize?product=all-i-know-is-win-tee');
  ...
  await expect(
    page.getByRole('button', { name: /Download Proof PDF|تحميل بروفة PDF/i }),
  ).toBeVisible();
  await expect(
    page.getByRole('button', { name: /Download Tech Pack|تحميل ملف التصنيع/i }),
  ).toBeVisible();
  await expect(
    page.getByRole('button', { name: /Production Artwork ZIP|حزمة ملفات الإنتاج ZIP/i }),
  ).toBeVisible();
});

test('secure design-share route loads through the protected server boundary', async ({ page }) => {
  const token = 'A'.repeat(48);
  await page.route('**/api/design-share?token=*', async (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
        ...
        design: {
          ...
          designData: {
            ...
            secondary: '#ffffff',
            accent: '#d4af37',
            studio: { activeView: 'front', layers: [], comments: [] },
          },
          comments: [],
        },
      }),
  ...
  await expect(
    page.getByRole('button', { name: /Approve Final Proof|اعتماد البروفة النهائية/i }),
  ).toBeVisible();
...
test('product discovery exposes stable related and recommendation cards', async ({ page }) => {
  await page.goto('/products/all-i-know-is-win-tee');
  await expect(page.locator('body')).not.toContainText(
    /Application error|isLowStock is not defined/,
  );
...
test('RTL, focus and mobile navigation stay usable on every primary public destination', async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 844 });
