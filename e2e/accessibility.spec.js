...
test('keyboard-only navigation reaches the main action and preserves visible focus', async ({
  page,
}) => {
  await page.goto('/');
...
test('modal traps focus, closes with Escape and returns focus to its trigger', async ({ page }) => {
  ...
  for (let index = 0; index < 8; index += 1) {
    await page.keyboard.press('Tab');
    expect(
      await page.locator(':focus').evaluate((node) => Boolean(node.closest('[role="dialog"]'))),
    ).toBeTruthy();
  }
...
test('checkout validation errors are announced and focus moves to an invalid field', async ({
  page,
}) => {
  await page.addInitScript(() => {
    localStorage.setItem(
      'shababuna-cart:guest',
      JSON.stringify([
        {
          key: 'product:p014:black-M-retail',
          type: 'product',
          id: 'p014',
          ...
          size: 'M',
          color: 'black',
          sku: 'LHA-TEE-WIN-BL-M',
          maxStock: 99,
          inventoryTracking: false,
          minQuantity: 1,
          href: '/products/all-i-know-is-win-tee',
          quantity: 1,
          purchaseMode: 'retail',
  ...
  await page
    .getByRole('button', { name: /Confirm Order|Place Pending Shipping Order|Pay/i })
    .click();
...
test('reduced motion disables smooth scrolling and RTL keeps logical reading order', async ({
  page,
}) => {
