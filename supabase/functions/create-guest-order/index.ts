...
const text = (value: unknown, max = 160) => typeof value === 'string' && value.trim().length > 0 && value.trim().length <= max;
...
Deno.serve(async (request) => {
  ...
  const { data: allowed, error: limitError } = await admin.rpc('consume_edge_rate_limit', {
    p_bucket: 'guest-create', p_subject_hash: subject, p_limit: 5, p_window_seconds: 900,
  });
